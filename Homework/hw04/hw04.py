...
def make_fib():
    ...
    second = 1 
    def fib():
        nonlocal first, second, zero
        output = first
        zero, first, second = first, second, first+second
        return output
    return fib 
...
def make_withdraw(balance, password):
    ...
    trials = 3
    passlist = []
    def withdraw(amount,text):
        nonlocal balance, trials
        if trials == 0:
            return 'Your account is locked. Attempts: ' + str(passlist)
        elif not text == password:
            trials -= 1 
            passlist.append(text)
...
def make_joint(withdraw, old_password, new_password):
    ...
    def newacc(amount,text):
        if text == old_password or text == new_password:
            return withdraw(amount,old_password)
        else:
            return withdraw(amount,text)
